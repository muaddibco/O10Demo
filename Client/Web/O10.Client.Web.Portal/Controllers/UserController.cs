...
namespace O10.Client.Web.Portal.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendUniversalProofs([FromQuery] long accountId, [FromBody] UniversalProofsSendingRequest request)
        {
            ...
            (var attributeByIssuer, var bf) = await CreateAttributesByIssuer(rootAttribute, persistency.RelationsBindingService, true).ConfigureAwait(false);
            ...
            RequestInput requestInput = new RequestInput
            {
                ...
                AssetCommitment = attributeByIssuer.RootAttribute.Commitment.ToByteArray(),
                BlindingFactor = bf
            };
            ...
            UniversalProofs universalProofs = new UniversalProofs
            {
                ...
                MainIssuer = attributeByIssuer.Issuer,
                RootIssuers = new List<RootIssuer>
                {
                    new RootIssuer
                    {
                        Issuer = attributeByIssuer.Issuer,
        ...
        private async Task SendUniversalTransport(long accountId, UtxoPersistency persistency, RequestInput requestInput, UniversalProofs universalProofs, string serviceProviderInfo, bool storeRegistration = false)
        {
            OutputModel[] outputModels = await _gatewayService.GetOutputs(_restApiConfiguration.RingSize + 1).ConfigureAwait(false);
        ...
        private async Task<bool> StoreRegistration(long accountId, UtxoPersistency persistency, byte[] target, string spInfo, Memory<byte> issuer, params Memory<byte>[] assetIds)
        {
        ...
        public async Task<IActionResult> CastVote(long accountId, [FromBody] VoteDto vote)
        {
            ...
            (var attributeByIssuer, _) = await CreateAttributesByIssuer(rootAttribute, persistency.RelationsBindingService, true).ConfigureAwait(false);
            (var attributeByIssuerPoll, var bf) = await CreateAttributesByIssuer(rootAttributePoll, persistency.RelationsBindingService).ConfigureAwait(false);

            UniversalProofs universalProofs = new UniversalProofs
            {
                SessionKey = ecCommitment.EcCommitment.ToHexString(),
                Mission = UniversalProofsMission.Vote,
                MainIssuer = attributeByIssuerPoll.Issuer,
                RootIssuers = new List<RootIssuer>
                {
                    new RootIssuer
                    {
                        Issuer = attributeByIssuerPoll.Issuer,
                        IssuersAttributes = new List<AttributesByIssuer>
                        {
                            attributeByIssuerPoll
                        }
                    }, new RootIssuer
                    {
                        Issuer = attributeByIssuer.Issuer,
            ...
            RequestInput requestInput = new RequestInput
            {
                ...
                PublicSpendKey = attributeByIssuerPoll.Issuer.ToByteArray(),
                AssetCommitment = attributeByIssuerPoll.RootAttribute.Commitment.ToByteArray(),
                BlindingFactor = bf
            };

            await SendUniversalTransport(accountId, persistency, requestInput, universalProofs, poll.Name).ConfigureAwait(false);
        ...
        private async Task<(AttributesByIssuer, byte[] bf)> CreateAttributesByIssuer(UserRootAttribute rootAttribute, IRelationsBindingService relationsBindingService, bool withProtectionAttribute = false)
        {
            // ================================================================================
            ...
            SurjectionProof proofToRegistration = await relationsBindingService.CreateProofToRegistration(issuer, bf, commitmentToRoot, rootAttribute.AssetId).ConfigureAwait(false);
            ...
            {
                var protectionAttribute = await _assetsService.GetProtectionAttributeProofs(bf,
                                                                                  rootAttribute.AssetId,
                                                                                  relationsBindingService,
                                                                                  rootAttribute.Source).ConfigureAwait(false);
                rootAttributeByIssuer.Attributes.Add(protectionAttribute);
            }
            // ================================================================================

            return (rootAttributeByIssuer, bf);
        }
    }
}